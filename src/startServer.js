...
if (isDebugMode) {
  console.log('Starting server in DEBUG mode for troubleshooting...');
} else {
...
const scriptToRun = isDebugMode ? './src/debugStart.ts' : './src/index.ts';
...
serverProcess.on('exit', (code, signal) => {
  if (code !== 0) {
    ...
    if (code === 1) {
      ...
      console.log('node src/startServer.js --debug');
    }
  } else {
    if (isDebugMode) {
